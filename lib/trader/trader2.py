  ...
  def doTrade(self):
    ...
    for i in range(self.n):
      ...
      for refP in ([self.nd[i, col.o], self.nd[i, col.h], self.nd[i, col.l], self.nd[i, col.c]] if self.ohlc else [self.nd[i, col.p]]):
        indices = self.nd[:i+1, :][self.nd[:i+1, col.reason] == exit_reasons.unknown & ((self.nd[:i+1, col.d] * (refP - self.nd[:i+1, col.sl])) <= 0), col.index]
        if len(indices) > 0:
          self.nd[indices, col.exit] = i
          self.nd[indices, col.xprice] = refP
          self.nd[indices, col.reason] = exit_reasons.sl
      # check tp
      for refP in ([self.nd[-1, col.o], self.nd[-1, col.h], self.nd[-1, col.l], self.nd[-1, col.c]] if self.ohlc else [self.nd[-1, col.p]]):
        indices = self.nd[:i+1, :][self.nd[:i+1, col.reason] == exit_reasons.unknown & ((self.nd[:i+1, col.d] * (refP - self.nd[:i+1, col.tp])) >= 0), col.index]
        if len(indices) > 0:
          self.nd[indices, col.exit] = i
          self.nd[indices, col.xprice] = refP
          self.nd[indices, col.reason] = exit_reasons.tp
      # check limit
      indices = self.nd[:i+1, :][self.nd[:i+1, col.reason] == exit_reasons.unknown & (self.nd[:i+1, col.entry] + self.nd[:i+1, col.limit] <= i), col.index]
      if len(indices) > 0:
        self.nd[indices, col.exit] = i
        self.nd[indices, col.xprice] = self.nd[i, col.c] if self.ohlc else self.nd[i, col.p]
        self.nd[indices, col.reason] = exit_reasons.limit
    # check for unfinished trades
    indices = self.nd[self.nd[:, col.reason] == exit_reasons.unknown, col.index]
    if len(indices) > 0:
      self.nd[indices, col.exit] = self.n - 1
      self.nd[indices, col.xprice] = self.nd[-1, col.c] if self.ohlc else self.nd[-1, col.p]
      self.nd[indices, col.reason] = exit_reasons.end
